...
2. Add this native code to request the push permission and let the SDK know when it is accepted, in the activity where you want to:
    ...
    class MainActivity : FlutterActivity() {
        ...
        override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
            ...
            if (requestCode == SelligentConstants.NOTIFICATION_PERMISSION_REQUEST_CODE && grantResults.size > 0 
                && grantResults.get(0) == PackageManager.PERMISSION_GRANTED) {
                FlutterselligentPlugin.enableNotifications()
        }
        }
    }
    ```
...
target 'ServiceExtension' do
  ...
  pod 'SelligentMobileSDK/FrameworkExtension', '3.8.5'
end
...
On iOS, if you want to customize the IAM styles, you can define the templates, to do so you will need to do it in the native part, in the `AppDelegate/didFinishLaunchingWithOptions` method by calling the `FlutterSelligent.setInAppMessageStylingTemplates` method, which expects an array of templates and (optionally) a default template id. If the default template is set, it will be used for all type of in-app messages (when a templateId wasn't provided or couldn't be found) and you will not need to use any specific templateId anywhere around your App.
...
| displayingIAM                    | string | DisplayingInAppMessage     | An IAM is about to be displayed (when `customInAppUi` is set to `true` in `selligent.json`) |
